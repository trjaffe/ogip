def ogip_dictionary_spectral():
    ...
    reqkeys = {
        ...
        "CREATOR":  {"level":3,"req":"h.Exists('CREATOR')"}, 
        # warn if another version
        "HDUVERS":  {"level":3,"req":"h.hasVal('HDUVERS','1.2.1')"},  
        "HDUCLAS2": { "level":3,"req":
        ...
        "POISSERR2": {"level":3,"req":"(h.hasVal('POISSERR',False) and h.hasCol('STAT_ERR')) or not h.hasCol('STAT_ERR') "},  
        #
    ...
    reqcols = {
        ...
        "ROWID":      {"level":3,"req":"not h.hasCol('SPEC_NUM') or h.hasCol('ROWID')"}, 
    }
